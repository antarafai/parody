...
interface ITNT721Receiver {
    function onTNT721Received(address operator, address from, uint256 tokenId, bytes calldata data) external returns (bytes4);
}
...
contract CoolNFT is TNT721 {
    ...
    modifier onlyOwner() {
        require(_msgSender() == _owner, "CoolNFT: caller is not the owner");
        _;
    }
    ...
    function mint(address to, uint256 tokenId, string memory uri) public onlyOwner {
        require(!_exists(tokenId), "CoolNFT: token already minted");
        _safeMint(to, tokenId);
